...
public:
    ...
    typedef int (XFThreadEntryPointProvider::*EntryMethodBody)(const void *);   ///< Prototype of method to be executed by the thread.
    ...
    /**
     * @brief Creates dynamically a new thread using provided parameters.
     * 
     * @param pProvider Pointer to instance providing the method to be executed by the thread.
     * @param entryMethod Address of method to be executed by the thread.
     * @param threadName Name to be given to the created thread.
     * @param stackSize Stack size in bytes. If size is 0, the default stack size is used during thread creation.
     */
    static interface::XFThread * create(interface::XFThreadEntryPointProvider * pProvider,
